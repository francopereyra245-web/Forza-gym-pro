...
export default function App(){
  ...
  useEffect(()=>{ try{ const p=localStorage.getItem("fp2"); if(p) setProds(JSON.parse(p)); const e=localStorage.getItem("fe2"); if(e) setEjs(JSON.parse(e)); }catch{} },[]);
  useEffect(()=>{ localStorage.setItem("fp2",JSON.stringify(prods)); },[prods]);
  useEffect(()=>{ localStorage.setItem("fe2",JSON.stringify(ejs)); },[ejs]);
