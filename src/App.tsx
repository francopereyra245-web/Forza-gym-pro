      ...
      <div className="min-h-screen bg-black text-white p-4 pb-24">
        ...
        <div className="text-center mt-4">
          ...
          <div className="mt-6 bg-zinc-900 rounded-2xl p-6 border border-zinc-800">
            <p className="text-6xl font-black tracking-wider">{formatTime(segundos)}</p>
            <p className="text-zinc-400 text-xs mt-1">TIEMPO DE DESCANSO</p>
            <div className="flex gap-2 mt-4 justify-center">
              <button onClick={()=>setSegundos(30)} className={`px-4 py-1.5 rounded text-xs font-bold ${segundos===30? 'bg-red-600' : 'bg-zinc-800'}`}>30s</button>
              <button onClick={()=>setSegundos(60)} className={`px-4 py-1.5 rounded text-xs font-bold ${segundos===60? 'bg-red-600' : 'bg-zinc-800'}`}>60s</button>
              <button onClick={()=>setSegundos(90)} className={`px-4 py-1.5 rounded text-xs font-bold ${segundos===90? 'bg-red-600' : 'bg-zinc-800'}`}>90s</button>
            </div>
          </div>
          ...
          <div className="mt-6 bg-zinc-900 rounded-xl p-4 border border-zinc-800 text-left">
            <div className="flex gap-3">
              <div className="flex-1">
                <label className="text-[10px] text-zinc-500 font-bold">PESO KG</label>
                <input value={pesos[ejercicioActual]||''} onChange={e=>setPesos({...pesos, [ejercicioActual]: e.target.value})} placeholder="80" className="w-full bg-black border border-zinc-700 rounded-lg p-3 mt-1 text-center font-bold text-white" />
              </div>
              <div className="flex-1">
                <label className="text-[10px] text-zinc-500 font-bold">REPS HECHAS</label>
                <input value={reps[ejercicioActual]||''} onChange={e=>setReps({...reps, [ejercicioActual]: e.target.value})} placeholder="10" className="w-full bg-black border border-zinc-700 rounded-lg p-3 mt-1 text-center font-bold text-white" />
              </div>
              <div className="flex items-end">
                <button onClick={()=>setChecks({...checks, [ejercicioActual]:!checks[ejercicioActual]})} className={`w-14 h-14 rounded-xl font-black text-xl border ${checks[ejercicioActual]? 'bg-green-600 border-green-600' : 'bg-zinc-800 border-zinc-700'}`}>✓</button>
              </div>
            </div>
          </div>
          ...
          <button
            onClick={()=> {
              ...
              else { alert('¡Rutina guardada! 🏆'); setVista('progreso'); setEjercicioActual(0) }
            }}
